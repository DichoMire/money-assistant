...
export function RateWarningModal({ group }: { group: GroupDto }) {
  ...
  return (
    <Modal title={t("rateWarning.title")} onClose={dismiss}>
      <div className="space-y-4 text-sm text-gray-600">
        {group.rates.latestDate ? (
          <p>
            {t("rateWarning.staleBefore", { currency: group.currency })}{" "}
            <span className="font-semibold">{formatDate(group.rates.latestDate, locale)}</span>{" "}
            {t("rateWarning.staleAfter", { days: RATES_STALE_DAYS })}
          </p>
        ) : (
          <p>{t("rateWarning.noneStored", { currency: group.currency })}</p>
        )}
        <p>{group.rates.latestDate ? t("rateWarning.inaccurate") : t("rateWarning.unavailable")}</p>
        {error && <p className="font-medium text-red-600">{error}</p>}
        <div className="flex justify-end gap-2 border-t border-gray-100 pt-4">
          <button type="button" className="btn btn-secondary" onClick={dismiss}>
            {t("rateWarning.dismiss")}
          </button>
          <button type="button" className="btn btn-primary" onClick={() => void refresh()} disabled={busy}>
            {busy ? t("rateWarning.updating") : t("rateWarning.updateNow")}
          </button>
        </div>
      </div>
    </Modal>
